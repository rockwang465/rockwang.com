...
function usage
{
    echo "usage: `basename $0` (-c || -s) [-o OSG_REPLICATION -k TOPIC_FACTOR -p PARTIIONS]"
    echo "   ";
...
function init
{
  kubectl exec -it -n component kafka-default-0 -- kafka-topics.sh --create --zookeeper zookeeper-default:2181/kafka -topic stream.images.face -replication-factor "${kafka_factor}" -partitions "${partitions}" --if-not-exists || true
  curl -X PUT -d '{"bucket_info":{"name":"image_face", "attrs":{"ttl":"90d" ,"replication":"'"${osg_replication}"'"}}}' http://0.0.0.0/components/osg-default/v1
}
