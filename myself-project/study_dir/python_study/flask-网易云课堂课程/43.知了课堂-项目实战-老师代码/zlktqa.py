...
def index():
    context = {
        # 获取数据中的所有数据，且按时间排序，且反转排序，create_time前面加个 "-"
...
@login_required
# 用装饰器判断：
# 如果存在session_id，表示登录，允许进入发布问答页面，即运行question()函数。
# 如果不存在session_id，表示未登录，跳转到登录页面。
def question():
    if request.method == 'GET':
        return render_template('question.html')
    else:
        # 这里为发布问答，单独建立一张数据表 question
        # 并且需要 python manage.py db migrate和upgrade数据库
