...
// 16.1 为了转异步写日志，定义了存放对应日志消息的内容的结构体
type logMsg struct {
...
type FileLogger struct {
	...
	logChan       chan *logMsg // 16.2 (channel类型的日志数据结构体)创建一个存放日志信息的channel通道，方便异步的时候去取值
}
...
func NewFileLog(lv, fileName, filePath string, maxLogSize int64) (fl *FileLogger) {
	...
	fl = &FileLogger{
		...
		logChan:    make(chan *logMsg, MaxSizeChan), // Rock注意7: 这里必须初始化才行
	}
...
// 16.5 将日志消息异步写入到文件中
func (fl *FileLogger) ChannelWriteLog() {
	select {
	case tmp := <-fl.logChan:
		fmt.Fprintf(fl.logFileObj, "[%s %s] [%s:%s:%v] : %s\n", tmp.now, tmp.strLogLevel, tmp.fileName, tmp.funcName, tmp.line, tmp.msg)
	}
}
...
func (fl *FileLogger) repeatInfo(lvInt logLevelInt, formatMsg string, a ...interface{}) {
	...
	//fmt.Fprintf(fl.logFileObj, "[%s %s] [%s:%s:%v] : %s\n", nowTime, strLogLevel, fileName, funcName, line, msg)  // 这个是原本直接写入到文件中的
	// 16.4 将日志信息保存到channel中
	tmp := &logMsg{
		now:         nowTime,
	...
	select {
	case fl.logChan <- tmp: //正常都能写进去的，因为里面的容量有50000
	default: // 如果特殊写不进去channel，则说明异常，但为了保证程序能运行，所以加了default
	}
